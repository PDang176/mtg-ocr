    ...
    def test_connection(self):
        try:
            with self.conn.cursor() as cur:
                ...
                tables = cur.fetchall()
                print(f"Tables in '{self.config.db_config['database']}':")
                for t in tables:
                    print(f"  - {t[0]}")
    ...
    def bulk_insert_cards(self):
        try:
            with self.conn.cursor() as cur:
                ...
                file_path = os.path.join(base_dir, "json", JSON_FILE_PATH)
                ...
                for i in range(0, total, BATCH_SIZE):
                    ...
                    rows = [self.__prepare_row(card) for card in batch]
...
if __name__ == "__main__":
    config = Config()
    db_creator = CreateDatabase(config)
    db_creator.test_connection()
    db_creator.bulk_insert_cards()